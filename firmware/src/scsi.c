...
void mac_scsi_check_phase (mac_scsi_t *scsi)
{
	...
	if (dif) {
#ifdef DEBUG_SCSI
		printf("scsi: phase mismatch (%02X)\n", dif);
#endif
...
{
#ifdef DEBUG_SCSI
	printf("scsi: phase: arbitration (id=%02X)\n", scsi->odr);
#endif
...
{
#ifdef DEBUG_SCSI
	printf("scsi: phase: status (%02X) cnt=%lu\n", val, scsi->buf_i);
#endif
...
void mac_scsi_cmd_read (mac_scsi_t *scsi, unsigned long lba, unsigned long cnt)
{
	...
	if (dsk->read(dsk, scsi->buf, lba, cnt)) {
		printf("scsi: read error at %lu + %lu\n", lba, cnt);
		mac_scsi_set_phase_status (scsi, 0x02);
		return;
	}
...
void mac_scsi_cmd_write_finish (mac_scsi_t *scsi, unsigned long lba, unsigned long cnt)
{
	...
	if ((512 * cnt) != scsi->buf_i) {
		printf("scsi: write size mismatch (%lu / %lu)\n",
			512 * cnt, scsi->buf_i
		);
		mac_scsi_set_phase_status (scsi, 0x02);
		return;
...
void mac_scsi_cmd_write6 (mac_scsi_t *scsi)
{
	...
	if (mac_scsi_set_buf_max (scsi, 512UL * cnt)) {
		printf("scsi: write block count %u\n", cnt);
		mac_scsi_set_phase_status (scsi, 0x02);
		return;
	}
...
void mac_scsi_cmd_mode_sense (mac_scsi_t *scsi)
{
	...
	default:
		printf("scsi: mode sense: unknown mode page (%02X)\n",
			scsi->cmd[2]
		);
		break;
	}
...
void mac_scsi_cmd_start_stop (mac_scsi_t *scsi)
{
	...
	printf("scsi: start/stop unit %u (%s)\n", scsi->sel_drv, str);
...
void mac_scsi_cmd_init (mac_scsi_t *scsi, unsigned char cmd)
{
	...
	default:
		printf("scsi: unknown command (%02X)\n", cmd);
		mac_scsi_set_phase_status (scsi, 0x02);
		break;
	}
}
...
#ifdef DEBUG_SCSI
			printf("scsi: command byte: %02X (%u/%u)\n",
...
static int dsk_adjust_chs (uint32_t *n, uint32_t *c, uint32_t *h, uint32_t *s)
{
	if (*n == 0) {
		*n = *c * *h * *s;
	}
